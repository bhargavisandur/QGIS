...
const rescuedbyMe = async (req, res) => {
    ...
    pool.query(
        'SELECT * FROM rescued_child WHERE ccid=$1', [crimecellId], async (err, result) => {

            if (err) throw err;
            else {
                let res1 = await pool.query(
                    'SELECT count(*) FROM rescued_child where ccid=$1', [crimecellId]
                );
...
const rescuedMale = async (req, res) => {
    ...
    var sex = 'Male';
    pool.query(
        'SELECT * FROM rescued_child WHERE ccid=$1 and "sex"=$2', [crimecellId, sex], async (err, result) => {
            if (err) throw err;
            else {
                let res1 = await pool.query(
                    'SELECT count(*) FROM rescued_child WHERE "sex"=$1 and ccid=$2 ', [sex, crimecellId]
                )
                let cnt = res1.rows[0].count;
    ...
    pool.query(
        'SELECT * FROM rescued_child WHERE ccid=$1 and "sex"=$2', [crimecellId, sex], async (err, result) => {
            if (err) throw err;
                ...
                let res1=await pool.query('SELECT count(*) FROM rescued_child WHERE "sex"=$1 and ccid=$2 ', [sex, crimecellId]);
                let cnt=res1.rows[0].count;
...
const rescuedPwd = async(req, res) => {
    ...
    console.log(crimecellId);
    pool.query(
        'SELECT * FROM rescued_child WHERE ccid=$1 and pwdstat=$2', [crimecellId, pwd], async(err, result) => {
            if (err) throw err;
            else{
                let res1=await pool.query('SELECT count(*) FROM rescued_child WHERE "pwdstat"=$1 and ccid=$2 ', [pwd, crimecellId]);
                let cnt=res1.rows[0].count;
...
const rescuedlt5 = async(req, res) => {
    ...
    console.log(crimecellId);
    pool.query(
        'SELECT * FROM rescued_child WHERE ccid=$1 and age=$2', [crimecellId, age], async(err, result) => {
            if (err) throw err;
            else{
                let res1=await pool.query('SELECT count(*) FROM rescued_child WHERE age=$1 and ccid=$2 ', [age, crimecellId]);
                let cnt=res1.rows[0].count;
...
module.exports = { getCrimeCell,displayCrime ,rescuedbyMe,rescuedMale,rescuedFemale,rescuedPwd,rescuedlt5,rescuedlt10,rescuedlt15};
